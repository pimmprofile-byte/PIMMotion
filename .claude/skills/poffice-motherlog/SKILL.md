...
- **카테고리 폴더 = 대시보드 카테고리와 동일**(구버전·이전 자료 확인용 아카이브 축):
  `미션보드/` · `게이트보드/` · `체크리스트/` · `세션로그/` · `워크오더/` · `사내공지/`
- **아카이빙**: `poffice_board.json` 갱신 시 **직전 버전/변경분을 해당 카테고리 폴더에** `{YYMMDD_HHMM}_{요약}` 로 남긴다(원문 무왜곡·불변, 원칙 2). 세션로그 원본은 블루필드/개인DB에 있고 여기 `세션로그/`는 집계 스냅샷·참조용.
