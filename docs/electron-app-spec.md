...
- **앱 진입**: 최신 에디터 HTML을 로드(단독). (허브를 진입점으로 할지는 2.0에서 결정 — 허브→에디터 링크 구조.)
- **HTML 소스**: 빌드 스텝에서 `tool/`의 최신 `PIMM_UnityGameCreator.verX.html`을 `desktop/renderer/index.html`로 복사(단일 소스 유지).
