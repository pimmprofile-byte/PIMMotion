...
- 세션로그 UX: **최근=선명 → 오래될수록 opacity 흐림 → 드롭다운/스크롤로 더 로드** (이메일 아닌 게임식). `poffice_board.json`은 최근 윈도우(**13h**+floor)+롤업만.
...
- **윈도우 = 시간 기준(하루 2회 예약)**: 최근 **13h**(주기 12h + 1h 겹침) 이내 세션·작업물 원문 + **인별 floor 1개**(13h 안에 없어도 최신 1개 유지). 기준 = 파일명 ts(KST). 밀린 원문은 개인폴더+`04_LogArchive`에 보존. 롤업은 상시 유지. 정본 규칙 = poffice-motherlog SKILL §3.
