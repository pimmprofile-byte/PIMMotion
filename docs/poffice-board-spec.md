...
## H. 스킬 2종 분리 (관심사·토큰 분리)
| 스킬 | 역할 | 트리거 |
|---|---|---|
| **`Poffice-Skill`** | 데이터 생성 — 개인세션로그·개인DB 취합 → `_board.json`(마더 세션로그) 자동생성, 자동예약 | 세션로그·세션·스케줄·작업보고·업무지시·업무보고·미션보드·포피스 / 포피스 업로드·수정 |
| **`Poffice-Report-Bot`** | 알림·브리핑 — 마더 세션로그를 직전 버전과 diff → 갱신 체크/미션/게이트·특이사항·사내공지를 **개인별 페르소나 말투**로 전달. 강제 아님, "{이름} 포피스알림켜줘"로 예약 활성화 | 포피스 알림·포피스알림켜줘·주간보고·일일보고·브리핑 |

- 페르소나(말투) 정본은 `.claude/skills/poffice-report-bot/SKILL.md` §4. 정보 정확성 > 캐릭터성(🔴·마감·리스크는 항상 명확).
- 두 스킬은 합치지 않음(토큰 낭비 방지). Report-Bot은 **읽기 전용**(데이터 생성은 Poffice-Skill).
